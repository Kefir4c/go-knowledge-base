...
ВАЖНО:  в новых реалиях мы не пишем SEIAL или BIGSERIAL,
Нужно писать два варианта автоматическо генерируемого уникального целочисленного значения:
id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY /если вам иногда нужно вручную задавать
id integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY /если вы хотите максимально защитить целостность данных от случайной ручной правки
Так же можно добавлять в конце можно настроить поведения последовательности:
1. START WITH <число> — начальное значение.
2. INCREMENT BY <число> — шаг приращения.
3. MINVALUE, MAXVALUE — границы диапазона.
4. ACHE <число> — сколько значений из последовательности будет предварительно кэшировано для производительности.

product_id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1000 INCREMENT BY 5),
