...
4.1. BUFFERS — твой главный друг
Выше я писал про: shared hit=100 read=50. Но это только верхушка айсберга.
Что на самом деле означают эти цифры:
hit — страница найдена в shared buffers (оперативная память PostgreSQL). Это быстро.
read — страница не найдена в кэше и была прочитана с диска (или из OS cache). Это медленно.

Почему это критично:
Если ты видишь read > 0 на больших таблицах — это признак того, что либо shared_buffers слишком маленький, либо запрос читает слишком много данных, которые не помещаются в память.
Золотое правило:
Всегда используй EXPLAIN (ANALYZE, BUFFERS), а не просто EXPLAIN ANALYZE. Это показывает реальную I/O-нагрузку, а не только время.

Пример:
EXPLAIN (ANALYZE, BUFFERS) SELECT * FROM users WHERE email = 'test@mail.com';

4.2. Heap Fetches в Index Only Scan
Выше написано, что Index Only Scan — это быстро, потому что таблица не читается. Но это не всегда так.

В плане Index Only Scan может появиться строчка:
text
Heap Fetches: 12345
Что это значит:
PostgreSQL не смог получить все данные из индекса и вынужден был сходить в таблицу. Это случается, когда visibility map говорит, 
что страница содержит «мёртвые» или невидимые для текущей транзакции строки.

Почему это происходит:
Таблица не была завакуумирована (VACUUM не обновил visibility map).
Строки были изменены после последнего VACUUM.

Что делать:
Выполнить VACUUM для таблицы, чтобы обновить visibility map, и индекс снова станет по-настоящему «only scan».

4.3. loops — скрытый убийца производительности
Выше написано loops=1 — сколько раз выполнялся узел.
Но если loops > 1 — это красный флаг.

Пример:
Nested Loop  (actual time=0.5..100.0 rows=1000 loops=1)
  -> Seq Scan on users  (actual time=0.1..0.2 rows=10 loops=1)
  -> Index Scan on orders  (actual time=0.1..9.9 rows=100 loops=10)
Здесь loops=10 означает, что Index Scan выполнился 10 раз — по одному разу на каждый найденный пользователь. 
Если бы пользователей было 1000, он выполнился бы 10000 раз. Это Nested Loop, и он может быть убийственным, если внешняя таблица большая.

Вывод:
Если видишь loops > 1 в узле, который читает много строк — это повод задуматься о переписывании запроса или создании другого индекса.

4.4. actual time vs cost — почему они не совпадают
Выше есть cost и actual time. Важно понимать: они измеряются в разных единицах.

cost — это условные единицы, которые планировщик использует для сравнения планов. Они не равны миллисекундам.
actual time — это реальное время в миллисекундах.

Почему это важно:
Если планировщик выбирает план с меньшим cost, но actual time оказывается больше — значит, статистика устарела. Нужно выполнить ANALYZE для таблицы.

4.5. Дополнительная статистика для узлов
PostgreSQL может показывать дополнительную информацию для некоторых узлов:
Для Sort: Sort Method: quicksort или external merge Disk. Если видишь Disk — сортировка не поместилась в work_mem и использовала диск. Это очень медленно.
Для Hash: Hash Batches: 2 — если > 1, значит, хэш-таблица не поместилась в память и использовала диск.

Что делать:
Увеличить work_mem для сессии (но осторожно, чтобы не убить сервер). 

5. ФИШКИ
1: «EXPLAIN ANALYZE изменяет данные для DML»
EXPLAIN ANALYZE реально выполняет запрос. Для INSERT, UPDATE, DELETE это значит, что данные изменятся!

Пример:
EXPLAIN ANALYZE DELETE FROM users WHERE id = 1; -- Удалит пользователя!

Как безопасно смотреть план для DML:
BEGIN;
EXPLAIN ANALYZE DELETE FROM users WHERE id = 1;
ROLLBACK; -- Откатываем изменения

2: «Bitmap Heap Scan — это компромисс»
Выше есть Bitmap Heap Scan. Когда он выбирается?

Index Scan — хорошо для малого числа строк (1-5% таблицы).
Seq Scan — хорошо для большого числа строк (>30% таблицы).
Bitmap Heap Scan — для среднего числа строк (5-30% таблицы).

Как он работает:
Bitmap Index Scan — проходит по индексу и создаёт битовую карту страниц, где лежат нужные строки.
Bitmap Heap Scan — читает эти страницы последовательно (а не по одной).

Почему это быстрее, чем Index Scan:
При Index Scan каждая строка читается отдельно (random I/O). При Bitmap Heap Scan страницы читаются блоками (sequential I/O), что быстрее на HDD и даже на SSD.
Но ещё необходимо учитывать время на формирование битовой карты.

3: «Типы JOIN'ов и когда они выбираются»
Выше есть теория про Nested Loop, Hash Join, Merge Join.

Nested Loop — для маленьких таблиц или когда есть индекс на внутренней таблице. Хорош, когда внешняя таблица даёт мало строк.
Hash Join — для больших таблиц, когда нет подходящих индексов. Строит хэш-таблицу в памяти (или на диске).
Merge Join — для отсортированных данных. Используется редко, потому что требует сортировки.

Что важно знать:
Если видишь Nested Loop с большим loops на внешней таблице — это признак проблемы. Нужно либо добавить индекс, либо переписать запрос, чтобы планировщик выбрал Hash Join.

4: «Проверка гипотез через EXPLAIN без ANALYZE»
Иногда ты хочешь проверить, будет ли использоваться индекс, но не хочешь ждать выполнения запроса (или он слишком тяжёлый).

Решение: Использовать EXPLAIN без ANALYZE. Он показывает план, который выбрал бы планировщик, но не выполняет запрос.
EXPLAIN (BUFFERS) SELECT * FROM users WHERE email = 'test@mail.com';
Это безопасно для DML (не изменяет данные) и быстро, потому что запрос не выполняется.
