...
6.1 ФИШКА ORDER BY:
КОЗЫРЬ №5: ORDER BY с NULLS FIRST / NULLS LAST
Это не прямо про порядок выполнения, но связано с ORDER BY. В PostgreSQL NULL по умолчанию считается больше любого значения
при сортировке по возрастанию (ASC), и меньше при сортировке по убыванию (DESC). Но это поведение можно изменить.
...
-- NULL в начале при сортировке по убыванию
SELECT name, salary FROM employees ORDER BY salary DESC NULLS FIRST;
Что говорить на собеседовании:

В PostgreSQL важно помнить про NULLS FIRST и NULLS LAST, потому что по умолчанию NULL считается больше всех при ASC и
меньше всех при DESC. Это часто приводит к неожиданному порядку, если не управлять этим явно.
