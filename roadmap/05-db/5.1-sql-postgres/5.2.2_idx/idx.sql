...
4.1. Обычный (один столбец)
Синтаксис:
CREATE INDEX idx_users_email ON users(email);
Когда применять:
Поиск по одному полю с =, >, <, >=, <=, BETWEEN, LIKE 'text%'.
Пример из жизни:
Таблица users с 1 млн строк. Частый запрос: SELECT * FROM users WHERE email = 'test@mail.com'; — создаём индекс на email.

4.2. Составной (по нескольким колонкам)
Синтаксис:
CREATE INDEX idx_orders_status_created ON orders(status, created_at);
Когда применять:
Если в WHERE есть несколько условий, которые часто идут вместе.

Правило порядка колонок (САМОЕ ВАЖНОЕ):
Индекс (a, b) ускоряет:
WHERE a = ...
WHERE a = ... AND b = ...
...
Как выбрать порядок:
* Сначала колонки с = (точное сравнение).
* Потом колонки с диапазонами (>, <, BETWEEN).
* Самую селективную (уникальную) колонку — первой.
Пример выбора порядка:
-- Запрос: найти доставленные заказы за январь 2023
SELECT * FROM orders
WHERE status = 'delivered' AND created_at BETWEEN '2023-01-01' AND '2023-01-31';

status — всего 3-4 значения (низкая селективность).
created_at — почти уникальные значения (высокая селективность).
Какой индекс лучше?

-- Вариант 1 (ПЛОХО)
CREATE INDEX idx_orders_status_created ON orders(status, created_at);
-- Сначала фильтруем по статусу (остаётся 30% строк), потом по дате.

-- Вариант 2 (ЛУЧШЕ)
CREATE INDEX idx_orders_created_status ON orders(created_at, status);
-- Сначала фильтруем по дате (остаётся 5% строк), потом по статусу.
Здесь created_at — более селективный, его первым.

4.3. Уникальный (UNIQUE INDEX)
Синтаксис:
CREATE UNIQUE INDEX idx_users_email_unique ON users(email);
Когда применять:
Гарантирует уникальность значений в колонке (или комбинации колонок). Работает как ограничение UNIQUE.

Пример:
CREATE UNIQUE INDEX idx_orders_user_product ON orders(user_id, product_id);
-- Один пользователь не может заказать один товар дважды.
Важно:
В PostgreSQL UNIQUE и CREATE UNIQUE INDEX — это одно и то же. При создании UNIQUE автоматически создаётся B-Tree индекс.

4.4. Частичный (Partial Index)
Синтаксис:
CREATE INDEX idx_users_active_email ON users(email) WHERE is_active = true;
Когда применять:
Если 80-90% запросов используют только подмножество строк. Например, все запросы ищут только активных пользователей, а неактивными почти не интересуются.

Пример:
-- Таблица users с 1 млн строк, из которых 50 000 активных.
-- Запрос: SELECT * FROM users WHERE is_active = true AND email = 'test@mail.com';
-- Частичный индекс на (email) WHERE is_active = true занимает в 20 раз меньше места и работает быстрее.
CREATE INDEX idx_users_active_email ON users(email) WHERE is_active = true;

Почему это круто:
Индекс физически меньше, быстрее сканируется, и UPDATE/INSERT работают быстрее (потому что индекс обновляется только для активных строк).

4.5. Индекс по выражению (Expression Index)
Синтаксис:
CREATE INDEX idx_users_email_lower ON users(LOWER(email));
Когда применять:
Когда в WHERE колонка обёрнута в функцию. Без такого индекса будет Seq Scan.

Пример:
-- Запрос: SELECT * FROM users WHERE LOWER(email) = 'test@mail.com';
-- Обычный индекс на email не поможет, потому что условие содержит функцию LOWER.
-- Создаём индекс по выражению:
CREATE INDEX idx_users_email_lower ON users(LOWER(email));

Важно:
Индекс по выражению поддерживает только точное совпадение с выражением. Если в запросе UPPER(email), а индекс на LOWER(email) — он не сработает.

4.6. Покрывающий (Covering Index) с INCLUDE
Синтаксис:
CREATE INDEX idx_users_active_cover ON users(is_active) INCLUDE (name, email);
Когда применять:
Когда запрос возвращает только несколько колонок, и ты хочешь, чтобы все данные брались из индекса, без обращения к таблице (Index Only Scan).

Пример:
-- Запрос: SELECT id, name, email FROM users WHERE is_active = true;
-- Создаём индекс, который покрывает все колонки в SELECT:
CREATE INDEX idx_users_active_cover ON users(is_active) INCLUDE (name, email);
-- Теперь PostgreSQL может выполнить Index Only Scan: все данные берутся из индекса, таблица не читается.

Важно:
Колонки в INCLUDE не участвуют в поиске (не влияют на WHERE), они просто хранятся в индексе для быстрого доступа.

4.7. GIN-индекс (для JSONB, массивов, полнотекстового поиска)
Синтаксис:
CREATE INDEX idx_products_attributes ON products USING GIN (attributes);
Когда применять:
Для поиска внутри JSONB, массивов, для полнотекстового поиска.

Пример:
-- Запрос: SELECT * FROM products WHERE attributes @> '{"color": "red"}';
CREATE INDEX idx_products_attributes ON products USING GIN (attributes);
...
7. ФИШКИ:
1: «Индекс не всегда используется, даже если он есть»
Почему:
Оптимизатор PostgreSQL оценивает стоимость операций. Если выборка возвращает >10-20% строк таблицы, 
индекс становится невыгодным (потому что нужно читать много строк по одному через индекс — это дорого, 
чем просто прочитать таблицу подряд). База выберет Seq Scan.

2: «Гипотетические индексы» (Hypothetical Indexes)
В чем суть: Это индексы, которые не существуют физически. Они не занимают место на диске, не тратят CPU и не замедляют запись.
Зачем: Ты можешь «создать» такой индекс в своей сессии, выполнить EXPLAIN и увидеть, изменится ли план запроса. 
Если да — ты знаешь, что реальный индекс поможет, и создаешь его. Если нет — не тратишь время и ресурсы.
Фишка: Это расширение hypopg. На собеседовании это покажет, что ты знаешь инструменты для безопасного экспериментирования, а не просто гадаешь.

Пример:
-- Создаем гипотетический индекс
SELECT * FROM hypopg_create_index('CREATE INDEX ON users (email)');
-- Смотрим план, будет ли использоваться
EXPLAIN SELECT * FROM users WHERE email = 'test@mail.com';
-- Если план изменился — создаем реальный индекс
CREATE INDEX CONCURRENTLY ON users (email);

3: «Частичный уникальный индекс» (Partial Unique Index)
В чем суть: Обычный UNIQUE-индекс запрещает дубликаты во всей таблице. Но ты можешь сделать UNIQUE только для части строк с помощью WHERE. 
Это решает задачи, где уникальность нужна не всегда.

Реальный пример: У тебя есть таблица users с колонкой email. Ты хочешь, чтобы только у активных пользователей (is_active = true) email был уникальным. 
Неактивные могут иметь дубли, они все равно не логинятся.

Фишка: Это решает бизнес-задачи, которые кажутся нерешаемыми без костылей в коде.
Пример:
-- Активные пользователи должны иметь уникальный email
CREATE UNIQUE INDEX idx_users_active_email 
ON users(email) 
WHERE is_active = true;

4: «Индексы с DESC и NULLS FIRST» — влияние на размер и скорость
В чем суть: По умолчанию B-Tree индекс строится в порядке ASC NULLS LAST. Но ты можешь изменить порядок на DESC NULLS FIRST.
Неочевидный эффект: Это влияет на размер индекса и скорость вставки. В одном из тестов индекс с DESC оказался в 2 раза больше и вставлялся на треть дольше.
Фишка: Если ты видишь, что индекс раздувается — попробуй поменять порядок сортировки. Иногда это дает неожиданный выигрыш.

Когда применять: Для таблиц, где данные вставляются в «правильном» порядке (например, монотонно возрастающие ID). 
Индекс с DESC будет хуже, потому что вставки идут в «левую» часть дерева, что вызывает частые перестройки.
Пример:
-- Может быть медленнее и больше
CREATE INDEX idx_bad ON table (id DESC);
-- Лучше для монотонных вставок
CREATE INDEX idx_good ON table (id ASC);

5: «Skip Scan» в составных индексах (PostgreSQL 18+)
В чем суть: Раньше составной индекс (status, date) помогал только для запросов с status или status + date. Запрос только по date игнорировал индекс.
С PostgreSQL 18 появился Skip Scan. Он позволяет индексу использоваться даже если первая колонка пропущена, но только если она имеет мало уникальных значений.

Фишка: Это новый инструмент, о котором знают немногие. Если ты упомянешь его на собеседовании — это покажет, что ты следишь за развитием Postgres.
Пример:
-- Индекс (status, date)
CREATE INDEX idx_orders_status_date ON orders(status, created_at);

-- Раньше этот запрос делал Seq Scan, теперь может использовать Skip Scan
